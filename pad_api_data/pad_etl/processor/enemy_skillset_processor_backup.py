...
def convert(enemy_behavior: List, level: int):
    ...
    if len(behavior_loops) > 0:
        ...
        # Extract all the behaviors of the loop
        if loop_size == 1:
            looping_behavior = turn_data[loop_start]
        else:
            looping_behavior = []
            for l in turn_data[loop_start:loop_end]:
                looping_behavior.extend(turn_data[l])

        # Trim the looped behavior from the 'timed' turns
        ...
        # Trim behavior from the repeating loop
        if loop_size > 1:
