class Describe_EN:
    ...
    @staticmethod
    def condition(chance, hp=None, one_time=False, extra_description=None):
        output = []
        ...
        if extra_description:
            output.append(extra_description)
    ...
    @staticmethod
    def bind(min_turns, max_turns, target_count=None, targets=None):
        output = []
        if not targets:
            targets = ['cards']
        if target_count:
            output.append('Bind {:d} {:s} cards'.format(target_count, ', '.join(targets)))
        else:
            output.append('Bind {:s} skills'.format(targets[0]))
    ...
    def row_col_spawn(position_type, positions, attributes):
        return 'Change {:s} {:s} to {:s} orbs'.format(
            ', '.join([Describe_EN.ordinal(x) for x in positions]), position_type, ', '.join(attributes))
    ...
    def random_orb_spawn(count, attributes):
        if count == 42:
            return Describe_EN.board_change(attributes)
        else:
    ...
    def orb_seal(turns, position_type, positions):
        return 'Seal {:s} {:s} for {:d} turns'\
            .format(', '.join([Describe_EN.ordinal(x) for x in positions]), position_type, turns)
    ...
    def cloud(turns, width, height, x, y):
        ...
        if x is not None and shape != 'Row of':
            pos.append('{:s} row'.format(Describe_EN.ordinal(x)))
        if y is not None and shape != 'Column of':
            pos.append('{:s} column'.format(Describe_EN.ordinal(y)))
        if len(pos) == 0:
    ...
    @staticmethod
    def enemy_remain(enemy_count, compare='<='):
        return 'when {} {} enemies remain'.format(compare, enemy_count)

    @staticmethod
    def post_status_effect():
        return 'after being affected by a status effect'

    @staticmethod
    def after_some_turns(turn_cd):
        return 'after {} turns'.format(turn_cd)

    @staticmethod
    def join_attr(attributes):
        return ', '.join(attributes)

    @staticmethod
    def combo_threshold(combo):
        return 'combo <= {:,d}'.format(combo)

    @staticmethod
    def invulnerable_off():
        return 'Remove damage immunity effect'

    @staticmethod
    def row_col_spawn_multi(position_type, positions, attributes):
        desc_arr = []
        for pos, att in zip(positions, attributes):
            desc_arr.append(Describe_EN.row_col_spawn(position_type, pos, att)[7:])
        return 'Change ' + ', '.join(desc_arr)

    @staticmethod
    def skillset():
        return 'Perform multiple skills'
    ...
    @staticmethod
    def random_position(count):
        return 'Random {:d}'.format(count)
