...
passive_stats_backups = {'for_attr': [], 'for_type': [], 'hp_multiplier': 1.0, 'atk_multiplier': 1.0, 'rcv_multiplier': 1.0, 'reduction_attributes': all_attr, 'damage_reduction': 0.0, 'skill_text': ''}   
def passive_stats_convert(arguments):
     def f(x):
          _,c = convert('passive_stats', {k:(arguments[k] if k in arguments else v) for k,v in passive_stats_backups.items()})(x)
          if not c['for_type']:
               if c['hp_multiplier'] != 1 and c['atk_multiplier'] != 1 and c['rcv_multiplier'] != 1:
                    if c['hp_multiplier'] == c['atk_multiplier'] == c['rcv_multiplier']:
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x all stats for '
                         ...
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & ATK and '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                         ...
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & RCV and '+str(c['atk_multiplier']).rstrip('0').rstrip('.') +'x ATK for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                         ...
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV and '+str(c['hp_multiplier']).rstrip('0').rstrip('.') +'x HP for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                    ...
                    if c['atk_multiplier'] == c['rcv_multiplier']:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                         ...
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and ' + str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                    ...
                    if c['hp_multiplier'] == c['rcv_multiplier']:
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & RCV for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                         ...
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP and ' + str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                    ...
                    if c['atk_multiplier'] == c['hp_multiplier']:
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & ATK for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
                         ...
                         c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP and ' + str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                         if c['for_attr'] == [0,1,2,3,4]:
                              c['skill_text'] += 'all Att.'
                         else:
                              for i in c['for_attr'][:-1]:
          ...
          if c['damage_reduction'] != 0.0:
               c['skill_text'] += '; Reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '%'
          return 'passive_stats' ,c
     return f

threshold_stats_backups = {'for_attr': [], 'for_type': [], 'threshold': False, 'atk_multiplier': 1.0, 'rcv_multiplier': 1.0, 'reduction_attributes': all_attr, 'damage_reduction': 0.0, 'skill_text': ''}
ABOVE = True
BELOW = False
def threshold_stats_convert(above, arguments):
    def f(x):
        if above:
            _,c = convert('above_threshold_stats', {k:(arguments[k] if k in arguments else v) for k,v in threshold_stats_backups.items()})(x)
            if c['damage_reduction'] == 0:
                if c['rcv_multiplier'] == 1.0:
                    if not c['for_type']:
                        c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK '
                        if c['for_attr'] == [0,1,2,3,4]:
                            c['skill_text'] += 'when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        else:
                            c['skill_text'] += 'for '
                            for i in c['for_attr'][:-1]:
                                c['skill_text'] += ATTRIBUTES[i] + ', '
                            c['skill_text'] += ATTRIBUTES[int(c['for_attr'][-1])] + ' Att. when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        return 'above_threshold_stats',c
                    else:
                        c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                        for i in c['for_type'][:-1]:
                            c['skill_text'] += TYPES[i] + ', '
                        c['skill_text'] += TYPES[int(c['for_type'][-1])] + ' type when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        return 'above_threshold_stats',c
                else:
                    if c['rcv_multiplier'] == c['atk_multiplier']:
                        if not c['for_type']:
                            c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV '
                            if c['for_attr'] == [0,1,2,3,4]:
                                c['skill_text'] += 'when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                            else:
                                c['skill_text'] += 'for '
                        ...
                        if not c['for_type']:
                            c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and ' + str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV '
                            if c['for_attr'] == [0,1,2,3,4]:
                                c['skill_text'] += 'when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                            else:
                                c['skill_text'] += 'for '
                ...
                c['skill_text'] += 'Reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.')+ '% when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                return 'above_threshold_stats',c
            else:
                if not c['for_type']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                    if c['for_attr'] == [0,1,2,3,4]:
                        c['skill_text'] += 'all Att. and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                    else:
                        for i in c['for_attr'][:-1]:
                            c['skill_text'] += ATTRIBUTES[i] + ', '
                        c['skill_text'] += ATTRIBUTES[int(c['for_attr'][-1])] + ' and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when above ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                    return 'above_threshold_stats',c
                else:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                    for i in c['for_type'][:-1]:
            ...
            _,c = convert('below_threshold_stats', {k:(arguments[k] if k in arguments else v) for k,v in threshold_stats_backups.items()})(x)
            if c['damage_reduction'] == 0:
                if c['rcv_multiplier'] == 1.0:
                    if not c['for_type']:
                        c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK '
                        if c['for_attr'] == [0,1,2,3,4]:
                            c['skill_text'] += 'when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        else:
                            c['skill_text'] += 'for '
                            for i in c['for_attr'][:-1]:
                                c['skill_text'] += ATTRIBUTES[i] + ', '
                            c['skill_text'] += ATTRIBUTES[int(c['for_attr'][-1])] + ' Att. when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        return 'below_threshold_stats',c
                    else:
                        c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                        for i in c['for_type'][:-1]:
                            c['skill_text'] += TYPES[i] + ', '
                        c['skill_text'] += TYPES[int(c['for_type'][-1])] + ' type when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                        return 'below_threshold_stats',c
                else:
                    if c['rcv_multiplier'] == c['atk_multiplier']:
                        if not c['for_type']:
                ...
                c['skill_text'] += 'Reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.')+ '% when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                return 'below_threshold_stats',c
            else:
                if not c['for_type']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                    if c['for_attr'] == [0,1,2,3,4]:
                        c['skill_text'] += 'all Att. and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                    else:
                        for i in c['for_attr'][:-1]:
                            c['skill_text'] += ATTRIBUTES[i] + ', '
                        c['skill_text'] += ATTRIBUTES[int(c['for_attr'][-1])] + ' and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when below ' + str(c['threshold']*100).rstrip('0').rstrip('.') + '% HP'
                    return 'below_threshold_stats',c
                else:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
                    for i in c['for_type'][:-1]:
...
def combo_match_convert(arguments):
    def f(x):
        ...
        if c['minimum_atk_multiplier'] == c['minimum_rcv_multiplier'] and c['bonus_atk_multiplier'] == c['bonus_rcv_multiplier']:
            c['skill_text'] = str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV when ' + str(c['minimum_combos']) + ' or more combos'
            if c['maximum_combos'] != c['minimum_combos']:
                c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(c['maximum_combos']-c['minimum_combos'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at ' + str(c['maximum_combos']) + ' combos'
        elif c['minimum_damage_reduction'] != 0:
            c['skill_text'] = str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by '+ str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when ' + str(c['minimum_combos']) + ' or more combos'
        else:
            c['skill_text'] = str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when ' + str(c['minimum_combos']) + ' or more combos'
            if c['maximum_combos'] != c['minimum_combos']:
                c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(c['maximum_combos']-c['minimum_combos'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at ' + str(c['maximum_combos']) + ' combos'
...
def attribute_match_convert(arguments):
    def f(x):
        ...
        if c['attributes'] == [0,1,2,3,4]:
            if c['minimum_atk_multiplier'] == c['minimum_rcv_multiplier'] and c['bonus_atk_multiplier'] == c['bonus_rcv_multiplier']:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV when matching ' + str(c['minimum_attributes']) + ' or more colors'
                if c['bonus_atk_multiplier'] != 0:
                    c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(5-c['minimum_attributes'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at 5 colors'
            elif c['minimum_damage_reduction'] != 0:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by '+ str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when matching ' + str(c['minimum_attributes']) + ' or more colors'
            else:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching ' + str(c['minimum_attributes']) + ' or more colors'
                if c['bonus_atk_multiplier'] != 0:
                    c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(5-c['minimum_attributes'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at 5 colors'
        elif c['attributes'] == [0,1,2,3,4,5]:
            if c['minimum_atk_multiplier'] == c['minimum_rcv_multiplier'] and c['bonus_atk_multiplier'] == c['bonus_rcv_multiplier']:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV when matching ' + str(c['minimum_attributes']) + ' or more colors (' + str(c['minimum_attributes']-1) + '+heal)'
                if c['bonus_atk_multiplier'] != 0:
                    c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(5-c['minimum_attributes'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at 5 colors'
            elif c['minimum_damage_reduction'] != 0:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by '+ str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when matching ' + str(c['minimum_attributes']) + ' or more colors (' + str(c['minimum_attributes']-1) + '+heal)'
            else:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching ' + str(c['minimum_attributes']) + ' or more colors (' + str(c['minimum_attributes']-1) + '+heal)'
                if c['bonus_atk_multiplier'] != 0:
                    c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(5-c['minimum_attributes'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at 5 colors'
        elif c['attributes'] != []:
            if c['minimum_atk_multiplier'] == c['minimum_rcv_multiplier']:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV when matching '
                for i in c['attributes'][:-1]:
                    c['skill_text'] += ATTRIBUTES[i] + ', '
                c['skill_text'] += ATTRIBUTES[int(c['attributes'][-1])]+ ' at once'
            else:
                c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching '
                for i in c['attributes'][:-1]:
                    c['skill_text'] += ATTRIBUTES[i] + ', '
                c['skill_text'] += ATTRIBUTES[int(c['attributes'][-1])] + ' at once'
...
def multi_attribute_match_convert(arguments):
     def f(x):
          _,c = convert('multi-attribute_match', {k:(arguments[k] if k in arguments else v) for k,v in multi_attribute_match_backups.items()})(x)
          if all(x == c['attributes'][0] for x in c['attributes']):
               if c['minimum_damage_reduction'] != 0 and c['minimum_rcv_multiplier'] == 1:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by ' + str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching ' + str(c['minimum_match']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' combos'
               elif len(c['attributes']) != c['minimum_match'] and c['minimum_rcv_multiplier'] == 1:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching ' + str(c['minimum_match']) + ' ' + ATTRIBUTES[c['attributes'][0]] + ' combos, up to ' + str(c['minimum_atk_multiplier']+(len(c['attributes'])-c['minimum_match'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at ' + str(len(c['attributes'])) + ' ' + ATTRIBUTES[c['attributes'][0]]+ ' combos'
               elif c['attributes'] != [] and c['minimum_rcv_multiplier'] == 1:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching ' + str(c['minimum_match']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' combos'
                    ...
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV and reduce damage taken by ' + str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching ' + str(c['minimum_match']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' combos'
               elif c['attributes'] != [] and c['minimum_rcv_multiplier'] != 1 and c['minimum_rcv_multiplier'] == c['minimum_atk_multiplier']:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV when matching ' + str(c['minimum_match']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' combos'
                    if len(c['attributes']) != c['minimum_match']:
                         c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(len(c['attributes'])-c['minimum_match'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x at ' + str(len(c['attributes'])) + ' ' + ATTRIBUTES[c['attributes'][0]]+ ' combos'
          else:
               if c['minimum_damage_reduction'] != 0 and c['minimum_rcv_multiplier'] == 1:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by ' + str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching '
                    z = False
                    ...
                    if z == True:
                         c['skill_text'] += ')'
                    if c['bonus_atk_multiplier'] != 0:
                         c['skill_text'] += ' up to ' + str(c['minimum_atk_multiplier']+(len(c['attributes'])-c['minimum_match'])*c['bonus_atk_multiplier']).rstrip('0').rstrip('.') + 'x when matching '
                    ...
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV and reduce damage taken by ' + str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching '
                    z = False
                    for i in range(0,len(c['attributes'])-int(c['minimum_match'])+1):
                         if i == len(c['attributes'])-int(c['minimum_match']) and i != 0:
                              c['skill_text'] += '('
                    ...
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['minimum_rcv_multiplier']).rstrip('0').rstrip('.')+'x RCV when matching '
                    z = False
                    for i in range(0,len(c['attributes'])-int(c['minimum_match'])+1):
                         if i == len(c['attributes'])-int(c['minimum_match']) and i != 0:
                              c['skill_text'] += '('
          ...
          if c['maximum_count'] == c['minimum_count']:
               if c['minimum_damage_reduction'] != 0:
                    if c['minimum_rcv_multiplier'] != 1.0 and c['minimum_rcv_multiplier'] == 'minimum_atk_multiplier':
                         c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and RCV and reduce damage taken by '+str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs'
                    elif c['minimum_rcv_multiplier'] != 1.0:
                         c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['minimum_rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV and reduce damage taken by '+str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs'
                    else:
                         c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by '+str(c['minimum_damage_reduction']*100).rstrip('0').rstrip('.') +'% when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs'

               else:
                    if c['minimum_rcv_multiplier'] != 1.0 and c['minimum_rcv_multiplier'] == 'minimum_atk_multiplier':
                         c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and RCV when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs'
                    elif c['attributes'] != []:
                         c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['minimum_rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs'
          else:
               if c['minimum_rcv_multiplier'] != 1.0 and c['minimum_rcv_multiplier'] == 'minimum_atk_multiplier':
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and RCV when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs up to ' + str((c['maximum_count']-c['minimum_count'])*c['bonus_atk_multiplier']+c['minimum_atk_multiplier']) + 'x at ' +str(c['maximum_count']) + ' orbs'
               elif c['minimum_rcv_multiplier'] != 1.0:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['minimum_rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs up to ' + str((c['maximum_count']-c['minimum_count'])*c['bonus_atk_multiplier']+c['minimum_atk_multiplier']) + 'x at ' +str(c['maximum_count']) + ' orbs'
               else:
                    c['skill_text'] += str(c['minimum_atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching ' + str(c['minimum_count']) + '+ ' + ATTRIBUTES[c['attributes'][0]] + ' orbs up to ' + str((c['maximum_count']-c['minimum_count'])*c['bonus_atk_multiplier']+c['minimum_atk_multiplier']) + 'x at ' +str(c['maximum_count']) + ' orbs'
          return 'mass_match',c
     return f
...
def after_attack_convert(arguments):
     def f(x):
          _,c = convert('after_attack_on_match', {k:(arguments[k] if k in arguments else v) for k,v in after_attack_on_match_backups.items()})(x)
          c['skill_text'] += str(c['multiplier']).rstrip('0').rstrip('.') + 'x ATK additional damage when matching orbs'
          return 'after_attack_on_match',c
     return f
heal_on_match_backups = {'multiplier':0, 'skill_text':''}
def heal_on_convert(arguments):
     def f(x):
          _,c = convert('heal on match', {k:(arguments[k] if k in arguments else v) for k,v in heal_on_match_backups.items()})(x)
          c['skill_text'] += str(c['multiplier']).rstrip('0').rstrip('.') + 'x RCV additional heal when matching orbs'
          return 'heal on match',c
     return f
resolve_backups = {'threshold':0, 'skill_text':''}
def resolve_convert(arguments):
     def f(x):
          _,c = convert('resolve', {k:(arguments[k] if k in arguments else v) for k,v in resolve_backups.items()})(x)
          c['skill_text'] += 'May survive when HP is reduced to 0 (HP>' + str(c['threshold']*100).rstrip('0').rstrip('.') + '%)'
          return 'resolve',c
     return f
bonus_move_time_backups = {'time': 0.0, 'for_attr': [], 'for_type': [], 'hp_multiplier': 1.0, 'atk_multiplier': 1.0, 'rcv_multiplier': 1.0, 'skill_text': ''}
def bonus_time_convert(arguments):
     def f(x):
          _,c = convert('bonus_move_time', {k:(arguments[k] if k in arguments else v) for k,v in bonus_move_time_backups.items()})(x)
          if not c['for_type']:
          ...
          if c['time'] != 0 and c['skill_text'] != '':
               c['skill_text'] += '; Increase orb movement time by ' + str(c['time']).rstrip('0').rstrip('.') + ' seconds'
          elif c['skill_text'] == '':
               c['skill_text'] += 'Increase orb movement time by ' + str(c['time']).rstrip('0').rstrip('.') + ' seconds'
...
def counter_attack_convert(arguments):
     def f(x):
          _,c = convert('counter_attack', {k:(arguments[k] if k in arguments else v) for k,v in counter_attack_backups.items()})(x)
          if c['chance'] == 1:
               c['skill_text'] += str(c['multiplier']).rstrip('0').rstrip('.') + 'x ' + ATTRIBUTES[int(c['attribute'])] + ' counterattack'
          else:
               c['skill_text'] += str(c['chance']*100).rstrip('0').rstrip('.') + '% chance to counterattack with ' + str(c['multiplier']).rstrip('0').rstrip('.') + 'x ' + ATTRIBUTES[int(c['attribute'])] + ' damage'
...
def egg_drop_convert(arguments):
     def f(x):
          _,c = convert('egg_drop_rate', {k:(arguments[k] if k in arguments else v) for k,v in egg_drop_backups.items()})(x)
...
def coin_drop_convert(arguments):
     def f(x):
          _,c = convert('coin_drop_rate', {k:(arguments[k] if k in arguments else v) for k,v in coin_drop_backups.items()})(x)
          c['skill_text'] += str(c['multiplier']).rstrip('0').rstrip('.') +'x Coin Drop rate'
          return 'coin_drop_rate',c
     return f
skill_used_backups = {'for_attr': [], 'for_type': [], 'atk_multiplier': 1, 'rcv_multiplier': 1, 'skill_text':''}
def skill_used_convert(arguments):
     def f(x):
          _,c = convert('skill_used_stats', {k:(arguments[k] if k in arguments else v) for k,v in skill_used_backups.items()})(x)
          if c['for_attr'] != []:
               if c['for_attr'] == [0,1,2,3,4]:
                    if c['rcv_multiplier'] != 1 and c['rcv_multiplier'] == c['atk_multiplier']:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV on the turn a skill is used'
                    elif c['rcv_multiplier'] != 1 and c['rcv_multiplier'] != c['atk_multiplier'] and c['atk_multiplier'] != 1:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV on the turn a skill is used'
                    elif c['rcv_multiplier'] == 1 and c['rcv_multiplier'] != c['atk_multiplier']:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK on the turn a skill is used'
                    elif c['atk_multiplier'] == 1 and c['rcv_multiplier'] != c['atk_multiplier']:
                         c['skill_text'] += str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV on the turn a skill is used'                         
               else:
                    if c['rcv_multiplier'] != 1 and c['rcv_multiplier'] == c['atk_multiplier']:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV for '
                         for i in c['for_attr'][:-1]:
                              c['skill_text'] += ATTRIBUTES[i] + ', '
                         c['skill_text'] += ATTRIBUTES[int(c['for_attr'][-1])] + ' Att. on the turn a skill is used'
                    elif c['rcv_multiplier'] != 1 and c['rcv_multiplier'] != c['atk_multiplier']:
                         c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV for '
                         for i in c['for_attr'][:-1]:
                              c['skill_text'] += ATTRIBUTES[i] + ', '
               ...
               if c['rcv_multiplier'] != 1 and c['rcv_multiplier'] == c['atk_multiplier']:
                    ...
                    for i in c['for_type'][:-1]:
                         c['skill_text'] += TYPES[i] + ', '                    
                    c['skill_text'] += TYPES[int(c['for_type'][-1])] + ' type on the turn a skill is used'
               elif c['rcv_multiplier'] != 1 and c['rcv_multiplier'] != c['atk_multiplier']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV for '
                    for i in c['for_type'][:-1]:
                         c['skill_text'] += TYPES[i] + ', '                    
...
def exact_combo_convert(arguments):
     def f(x):
          _,c = convert('exact_combo_match', {k:(arguments[k] if k in arguments else v) for k,v in exact_combo_backups.items()})(x)
          c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') +'x ATK when exactly '+ str(c['combos']) + ' combos'
          return 'exact_combo_match',c
     return f
passive_stats_type_atk_all_hp_backups = {'for_type': [], 'atk_multiplier': 1, 'hp_multiplier': 1, 'skill_text': ''}
def passive_stats_type_atk_all_hp_convert(arguments):
     def f(x):
          _,c = convert('passive_stats_type_atk_all_hp', {k:(arguments[k] if k in arguments else v) for k,v in passive_stats_type_atk_all_hp_backups.items()})(x)          
          c['skill_text'] += 'Reduce total HP by ' + str((1-c['hp_multiplier'])*100).rstrip('0').rstrip('.') + '%; ' + str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for '
          for i in c['for_type'][:-1]:
               c['skill_text'] += TYPES[i] + ', '                    
          c['skill_text'] += TYPES[int(c['for_type'][-1])] + ' type'
          return 'passive_stats_type_atk_all_hp',c
     return f
team_build_bonus_backups = {'monster_ids': 0, 'hp_multiplier': 1, 'atk_multiplier': 1, 'rcv_multiplier': 1, 'skill_text': ''}
def team_build_bonus_convert(arguments):
     def f(x):
          _,c = convert('team_build_bonus', {k:(arguments[k] if k in arguments else v) for k,v in team_build_bonus_backups.items()})(x)          
          if c['hp_multiplier'] != 1 and c['atk_multiplier'] != 1 and c['rcv_multiplier'] != 1:
               if c['hp_multiplier'] == c['atk_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x all stats if ' + str(c['monster_ids']) + ' is on the team'
               elif c['hp_multiplier'] == c['atk_multiplier']:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & ATK and '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV if ' + str(c['monster_ids']) + ' is on the team'
               elif c['hp_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & RCV and '+str(c['atk_multiplier']).rstrip('0').rstrip('.') +'x ATK if ' + str(c['monster_ids']) + ' is on the team'                    
               elif c['atk_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV and '+str(c['hp_multiplier']).rstrip('0').rstrip('.') +'x HP if ' + str(c['monster_ids']) + ' is on the team'                    
          elif c['hp_multiplier'] == 1 and c['atk_multiplier'] != 1 and c['rcv_multiplier'] != 1:
               if c['atk_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV if ' + str(c['monster_ids']) + ' is on the team'
               else:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and ' + str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV if ' + str(c['monster_ids']) + ' is on the team'
          elif c['hp_multiplier'] != 1 and c['atk_multiplier'] == 1 and c['rcv_multiplier'] != 1:
               if c['hp_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & RCV if ' + str(c['monster_ids']) + ' is on the team'
               else:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP and ' + str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV if ' + str(c['monster_ids']) + ' is on the team'
          elif c['hp_multiplier'] != 1 and c['atk_multiplier'] != 1 and c['rcv_multiplier'] == 1:
                if c['atk_multiplier'] == c['hp_multiplier']:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP & ATK if ' + str(c['monster_ids']) + ' is on the team'
                else:
                    c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP and ' + str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK if ' + str(c['monster_ids']) + ' is on the team'
          elif c['hp_multiplier'] == 1 and c['atk_multiplier'] == 1 and c['rcv_multiplier'] != 1:
                c['skill_text'] += str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV if ' + str(c['monster_ids']) + ' is on the team'
          elif c['hp_multiplier'] == 1 and c['atk_multiplier'] != 1 and c['rcv_multiplier'] == 1:
                c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK if ' + str(c['monster_ids']) + ' is on the team'
          elif c['hp_multiplier'] != 1 and c['atk_multiplier'] == 1 and c['rcv_multiplier'] == 1:
                c['skill_text'] += str(c['hp_multiplier']).rstrip('0').rstrip('.') + 'x HP if ' + str(c['monster_ids']) + ' is on the team'       
          return 'team_build_bonus',c
     return f
rank_exp_rate_backups = {'multiplier': 1, 'skill_text':''}
def rank_exp_rate_convert(arguments):
     def f(x):
          _,c = convert('rank_exp_rate', {k:(arguments[k] if k in arguments else v) for k,v in rank_exp_rate_backups.items()})(x)
...
def heart_tpa_stats_convert(arguments):
     def f(x):
          _,c = convert('heart_tpa_stats', {k:(arguments[k] if k in arguments else v) for k,v in heart_tpa_stats_backups.items()})(x)
          c['skill_text'] += str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV when matching 4 Heal orbs'
...
def five_orb_one_enhance_convert(arguments):
     def f(x):
          _,c = convert('five_orb_one_enhance', {k:(arguments[k] if k in arguments else v) for k,v in five_orb_one_enhance_backups.items()})(x)
          c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK for matched Att. when matching 5 Orbs with 1+ enhanced'
          return 'five_orb_one_enhance',c
     return f
heart_cross_backups = {'atk_multiplier': 1, 'rcv_multiplier': 1, 'damage_reduction': 0, 'skill_text':''}
def heart_cross_convert(arguments):
     def f(x):
          _,c = convert('heart_cross', {k:(arguments[k] if k in arguments else v) for k,v in heart_cross_backups.items()})(x)
          if c['atk_multiplier'] != 1 and c['rcv_multiplier'] != 1 and c['damage_reduction'] != 0:
               if c['atk_multiplier'] == c['rcv_multiplier']:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK & RCV and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching 5 Heal orbs in a cross fromation'
               else:
                    c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK, '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] != 1 and c['rcv_multiplier'] == 1 and c['damage_reduction'] != 0:
               c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] == 1 and c['rcv_multiplier'] != 1 and c['damage_reduction'] != 0:
               c['skill_text'] += str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV and reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] != 1 and c['rcv_multiplier'] != 1 and c['damage_reduction'] == 0:
               c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK, '+str(c['rcv_multiplier']).rstrip('0').rstrip('.') +'x RCV when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] != 1 and c['rcv_multiplier'] == 1 and c['damage_reduction'] == 0:
               c['skill_text'] += str(c['atk_multiplier']).rstrip('0').rstrip('.') + 'x ATK when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] == 1 and c['rcv_multiplier'] != 1 and c['damage_reduction'] == 0:
               c['skill_text'] += str(c['rcv_multiplier']).rstrip('0').rstrip('.') + 'x RCV when matching 5 Heal orbs in a cross fromation'
          elif c['atk_multiplier'] == 1 and c['rcv_multiplier'] == 1 and c['damage_reduction'] != 0:
               c['skill_text'] += 'Reduce damage taken by ' + str(c['damage_reduction']*100).rstrip('0').rstrip('.') + '% when matching 5 Heal orbs in a cross fromation'    
