...
class Exchange(pad_util.JsonDictEncodable):
    ...
    def __repr__(self):
        return 'Exchange({} - {} - {}/{})'.format(self.monster_id, len(self.required_monsters), self.start_time_str, self.end_time_str)
...
def load_data(data_dir: str=None, json_file: str=None, server: str=None) -> List[Exchange]:
    ...
    if data_json['v'] > 1:
        print('Warning! Version of exchange file is not tested: {}'.format(data_json['v']))
